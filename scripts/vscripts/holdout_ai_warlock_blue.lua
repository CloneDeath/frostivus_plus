...
function DispatchOnPostSpawn()
       ...
       behaviorSystem = AICore:CreateBehaviorSystem( { BehaviorNone, BehaviorShadowWord, BehaviorTransmogrify, BehaviorChaoticOffering, BehaviorRunAway } ) 
...
BehaviorTransmogrify = {}

function BehaviorTransmogrify:Evaluate()
	local desire = 0

	-- let's not choose this twice in a row
	if AICore.currentBehavior == self then return desire end

	self.transmogrifyAbility = thisEntity:FindAbilityByName( "creature_warlock_transmogrify" )
	local enemies = FindUnitsInRadius( DOTA_TEAM_BADGUYS, thisEntity:GetOrigin(), nil, self.transmogrifyAbility:GetCastRange(), DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO, 0, 0, false )
	if #enemies > 0 then
		for _,enemy in pairs(enemies) do
			local transmogrifyBuff = enemy:HasModifier( "modifier_creature_warlock_transmogrify" )
			if not transmogrifyBuff then
				self.target = enemy
				desire = 5
				break
			end
...
function BehaviorTransmogrify:Begin()
	self.duration = 5
	...
	self.order =
	{
		...
		UnitIndex = thisEntity:entindex(),
		TargetIndex = self.target:entindex(),
		AbilityIndex = self.transmogrifyAbility:entindex()
	}
end

BehaviorTransmogrify.Continue = BehaviorTransmogrify.Begin --if we re-enter this ability, we might have a different target; might as well do a full reset
...
AICore.possibleBehaviors = { BehaviorNone, BehaviorShadowWord, BehaviorTransmogrify, BehaviorChaoticOffering, BehaviorRunAway }
