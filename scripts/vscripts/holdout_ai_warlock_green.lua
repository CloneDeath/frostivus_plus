...
function DispatchOnPostSpawn()
       ...
       behaviorSystem = AICore:CreateBehaviorSystem( { BehaviorNone, BehaviorShadowWord, BehaviorUpheaval, BehaviorChaoticOffering, BehaviorRunAway } ) 
...
function BehaviorUpheaval:Evaluate()
	local desire = 0
	
	-- let's not choose this twice in a row
	if currentBehavior == self then return desire end

	self.upheavalAbility = thisEntity:FindAbilityByName( "warlock_upheaval" )
	
	if self.upheavalAbility and self.upheavalAbility:IsFullyCastable() then
		self.target = AICore:RandomEnemyHeroInRange( thisEntity, self.upheavalAbility:GetCastRange() )
		if self.target then
...
function BehaviorUpheaval:Begin()
	self.duration = 12
	...
	self.order =
	{
		...
		AbilityIndex = self.upheavalAbility:entindex(),
		Position = targetPoint
	}

end

BehaviorUpheaval.Continue = BehaviorUpheaval.Begin
...
AICore.possibleBehaviors = { BehaviorNone, BehaviorShadowWord, BehaviorUpheaval, BehaviorChaoticOffering, BehaviorRunAway }
